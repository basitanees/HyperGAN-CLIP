...
from models.DynaGAN_nada import DynaGAN
from utils.file_utils import save_images
from options.DynaGAN_options_nada import DynaGANOptions
from datasets.image_dataset_ffhq import ImagesDataset
...
def train(args, output_dir):
    ...
    config = vars(args)
    wandb.init(project="Domain-adaptation",config=config, settings=wandb.Settings(_service_wait=300))
    print("Wandb init")
    ...
    # for i, batch_target in pbar:
            ...
            if (i % args.vis_interval == 0 or (i + 1) == args.iter):
                ...
                target = target_L
                with torch.no_grad():
                    target_projected_w_res = net.encoder(target_L)
                    target_projected_w_res[:,:7,:] *= 0.7
                    target_projected_w_res[:,7:,:] *= 0.4
                    target_projected_w = net.generator_frozen.mean_latent[None,None,:] + target_projected_w_res
...
if __name__ == "__main__":
    ...
    parser.add_argument('--e4e_checkpoint_path', type=str, default="/kuacc/users/aanees20/hpc_run/DynaGAN/pretrained_models/e4e_ffhq_encode.pt")
    # parser.add_argument("--iter", type=int, default=200, help="num epochs")
