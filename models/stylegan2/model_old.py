...
class HyperNet(nn.Module):
    ...
    def forward(self, domain_style_vec):
        batch = len(domain_style_vec)
        
        domain_style = domain_style_vec
        if self.scaled is not None:
            if self.scaled["type"] == "linear":
                domain_style = domain_style_vec * self.sigmoid(self.dom_scale(domain_style_vec))
            elif self.scaled["type"] == "dot":
                domain_style = self.dom_scale.forward_dot(domain_style_vec)
                # domain_style_norm = domain_style / domain_style.clone().norm(dim=-1, keepdim=True)
                # dom_scale = self.dom_scale / self.dom_scale.clone().norm(dim=-1, keepdim=True)
                # # scale = self.cos_sim(domain_style_norm, dom_scale)[:,None]
                # scale = torch.acos(self.cos_sim(domain_style_norm, dom_scale)[:,None]) / torch.acos(torch.zeros(1)).item() * 2
...
class Generator(nn.Module):
    ...
    def create_domain_modulation(self, no_scaling=False, no_residual=False, scaled=None, embed2embed=False):
