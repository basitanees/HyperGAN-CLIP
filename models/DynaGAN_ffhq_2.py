...
from models.stylegan2.model import Generator #model_3
...
#[conv.conv.residual_multiplier for conv in self.generator.convs] + 
    ...
    def forward(self,
        ...
        return self.generator(styles, return_latents=return_latents, truncation=truncation,
                              ...
                              domain_is_latents=domain_is_latents, multipliers=multipliers*self.num_layers)
...
class DynaGAN(torch.nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            domain_labels1 = domain_labels#[domain_labels[0].clone()]
            domain_labels2 = domain_labels#[domain_labels[0].clone()]
        if inference:
            with torch.no_grad():
